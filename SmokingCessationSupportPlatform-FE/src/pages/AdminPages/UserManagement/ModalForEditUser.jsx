...
const UserInfoHeader = ({ userData }) => (
  <div className={styles.userHeader}>
    ...
    <div className={styles.userInfo}>
      <Title level={4} className={styles.userName}>{userData.fullName}</Title>
      <div className={styles.userIdContainer}>
        <Text type="secondary">User ID: #{userData.userId}</Text>
        <span className={`${styles.statusBadge} ${!userData.isBlock ? styles.statusActive : styles.statusBlocked}`}>
...
const BasicInfoForm = () => (
  <>
    <Title level={5} className={styles.sectionTitle}>Basic Information</Title>
    <Row gutter={16}>
      <Col span={12}>
      ...
      <Col span={12}>
        <Form.Item label="Profile Name" name="profileName" rules={[{ required: true, message: 'Please enter profile name' }]}>
...
const AccountStatusForm = () => (
  <>
    <Title level={5} className={styles.sectionTitle}>Account Status</Title>
    <div className={styles.statusSection}>
      <Form.Item name="isActive" valuePropName="checked" className={styles.switchItem}>
      ...
      <Form.Item name="isVerified" valuePropName="checked" className={styles.switchItem}>
        <div className={styles.switchWrapper}>
          <div className={styles.switchInfo}>
            <Text strong>Email Verified</Text>
            <Text type="secondary">Indicates if the user's email is verified.</Text>
          </div>
          <Switch />
        </div>
      </Form.Item>
...
const SystemInfo = ({ userData }) => (
  <>
    <Title level={5} className={styles.sectionTitle}>System Information</Title>
    <Row gutter={16}>
      <Col span={8}><div className={styles.infoItem}><Text type="secondary">Created:</Text><Text strong>{userData.createdAt ? dayjs(userData.createdAt).format('DD/MM/YYYY') : 'N/A'}</Text></div></Col>
      <Col span={8}><div className={styles.infoItem}><Text type="secondary">Last Updated:</Text><Text strong>{userData.updatedAt ? dayjs(userData.updatedAt).format('DD/MM/YYYY') : 'N/A'}</Text></div></Col>
      <Col span={8}><div className={styles.infoItem}><Text type="secondary">Last Login:</Text><Text strong>{userData.lastLogin ? dayjs(userData.lastLogin).format('DD/MM/YYYY') : 'N/A'}</Text></div></Col>
    </Row>
  </>
);
...
const ModalFooter = ({ onCancel, submitting }) => (
  <div className={styles.footer}>
    <Button icon={<CloseOutlined />} onClick={onCancel} disabled={submitting}>Cancel</Button>
    <Button type="primary" htmlType="submit" loading={submitting} icon={<SaveOutlined />}>{submitting ? 'Saving...' : 'Save Changes'}</Button>
  </div>
);
...
const ModalForEditUser = ({ open, onClose, userId, onUserUpdated }) => {
  ...
  return (
    <Modal title={null} open={open} onCancel={handleClose} footer={null} width={700} className={styles.editModal}>
      {loading ? (
        <div className={styles.loadingWrapper}><Spin size="large" /></div>
      ) : userData ? (
        <div className={styles.modalContent}>
          <Title level={3} className={styles.title}>Edit User Information</Title>

...
const ScheduleManagement = () => {
  ...
  const fetchSchedules = useCallback(async () => {
    ...
    try {
      ...
      const filteredSchedules = selectedMentor 
        ? mockSchedules.filter(schedule => schedule.mentorEmail === selectedMentor)
        : mockSchedules;
  ...
  // Handler for batch slot creation
  const handleBatchCreateSlots = async (slots) => {
    try {
      ...
      // Add new slots to existing schedules
      const newSlots = slots.map((slot, index) => ({
        ...
        mentorEmail: slot.mentorEmail,
        mentorName: mentorOptions.find(m => m.email === slot.mentorEmail)?.name || 'Unknown',
        status: 'available',
      ...
      message.success(`Successfully created ${slots.length} consultation slots`);
    } catch (error) {
          ...
          prev.map(schedule => 
            schedule.id === currentSlot.id 
              ? { 
                  ...schedule, 
                  ...updatedSlot,
                  mentorName: mentorOptions.find(m => m.email === updatedSlot.mentorEmail)?.name || 'Unknown'
                }
              : schedule
          )
        );
  ...
  return (
    <AdminLayout title="Schedule Management" showBreadcrumb={false}>
      <div style={{ background: '#f7f8fa', minHeight: '100vh', padding: 0 }}>
        {/* Header */}
        <Card style={{ borderRadius: 16, margin: '24px 0 0 0', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }} bodyStyle={{ padding: 32, paddingBottom: 20 }}>
          <Row align="middle" justify="space-between" gutter={[16, 16]}>
            <Col>
              <Space align="center">
                <CalendarOutlined style={{ fontSize: 32, color: '#3b82f6' }} />
                <div>
                  <div style={{ fontSize: 28, fontWeight: 700, color: '#222', marginBottom: 2 }}>Schedule Management</div>
                  <div style={{ fontSize: 15, color: '#64748b' }}>Manage consultation schedules and mentor availability</div>
                </div>
              </Space>
            </Col>
            <Col>
        ...
        <div style={{ maxWidth: 1400, margin: '32px auto', padding: '0 16px' }}>
          <Card style={{ borderRadius: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }} bodyStyle={{ padding: 32 }}>
            <ScheduleTable
              schedules={schedules}
              loading={loading}
